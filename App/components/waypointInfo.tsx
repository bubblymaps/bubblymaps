...
export default function WaypointInfoPanel({
    ...
    return (
        <div
            className="fixed top-20 left-17 z-10 w-96 bg-white shadow-xl rounded-2xl border border-gray-200 flex flex-col max-h-[calc(100vh-2rem)] overflow-hidden">
            <MagicLinkPopup
                isOpen={isPopupOpen}
                ...
                <div className="absolute top-3 right-3 flex gap-2">
                    ...
                    {currentUserId && (
                        <>
                            ...
                            <button
                                onClick={async () => {
                                    const reason = prompt("Why report this waypoint?");
                                    if (!reason) return;
                                    ...
                                    try {
                                        ...
                                        alert("Waypoint reported successfully! Thanks for your feedback.");
                                    } catch (err: any) {
                                        alert(err.message);
                                    }
                                }}
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      <span className="font-medium">
                        {avgRating.toFixed(1)} ({reviews.length})
                                ...
                                {selectedWaypoint.verified && avgRating >0 && (
                                    <span>•</span>
                                )}
                    ...
                    {tabs.map((tab) => (
                        ...
                        >
                            ...
                            {tab.badge !== undefined && tab.badge > 0 && (
                                <span
                                    className="ml-1 px-2 py-0.5 text-xs bg-gray-100 text-gray-600 rounded-full">{tab.badge}</span>
                            )}
                        </button>
                    ))}
                </div>
        ...
        <div className="p-4 space-y-6">
            ...
            <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl space-y-2 text-sm text-gray-500">
                ...
                {selectedWaypoint.addedby && (
                    <div>
                        Added by{" "}
                        <Link href={`/user/${selectedWaypoint.addedby}`}
                ...
                {selectedWaypoint.createdAt && (
                    <div>
                        Added on{" "}
                        <strong
                            className="text-gray-700">{new Date(selectedWaypoint.createdAt).toLocaleDateString()}</strong>
                    </div>
                )}
                ...
                {selectedWaypoint.updatedAt && (
                    <div>
                        Last updated on{" "}
                        <strong
                            className="text-gray-700">{new Date(selectedWaypoint.updatedAt).toLocaleDateString()}</strong>
                    </div>
                )}
            </div>
        </div>
            ...
            {!userReview && (
                <>
                    {currentUserId ? (
                        ...
                        <div className="p-4 text-center text-gray-500 space-y-4 border border-gray-200 rounded-xl">
                                    ...
                                    <button
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                if (!res.ok) throw new Error("Failed to report review. Are you signed in?");
                                                alert("Review reported successfully! Thanks for keeping our community safe.");
                                            } catch (err: any) {
                                                alert(err.message);
                                            }
                                        }}
                ...
                <>
                  ...
                  >
                    {log.action}
                                    ...
                                    <span
                                        className="text-xs text-gray-500">{new Date(log.createdAt).toLocaleDateString()}</span>
                                </div>
                                        ...
                                        : Object.entries(log.changes)
                                            ...
                                            .map(([field, change]: any) => (
                                                <div key={field} className="text-sm">
                                                    <span className="font-medium text-gray-700">{field}:</span>{" "}
                                                    <span
                                                        className="text-red-600 line-through">{String(change.old)}</span>
                                                    {" → "}
                                                    <span
                                                        className="text-green-600 font-medium">{String(change.new)}</span>
                                                </div>
                                            ))}
                                </div>
                            </div>
            ...
            <div className="flex flex-col gap-2">
                {messages.length === 0 && (
                    <div className="text-center py-4">
                        <p className="text-gray-500">Ask Bubbly</p>
                        <p className="text-sm text-gray-400">
